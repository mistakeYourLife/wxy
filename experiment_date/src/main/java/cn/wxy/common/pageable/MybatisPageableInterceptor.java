...
public class MybatisPageableInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation inv) throws Throwable {
        ...
        // 查找方法参数中的 分页请求对象
        Page pageRequest = this.findPageableObject(queryArgs[PARAMETER_INDEX]);
        ...
        if(pageRequest != null) {
            ...
            // 2. 搞定limit 查询
            // 2.1 获取分页SQL，并完成参数准备
            String limitSql = dialect.getLimitString(sql, pageRequest.getiDisplayStart(), pageRequest.getiDisplayLength());
            ...
            //pi.setiTotalDisplayRecords(pageRequest.getPageSize());
            pageRequest.setData((List) ret);
            pageRequest.setiTotalDisplayRecords(total);
            pageRequest.setiTotalRecords(total);
